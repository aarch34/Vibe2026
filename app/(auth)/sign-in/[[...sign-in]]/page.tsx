...
import { Sparkles, ArrowRight, ShieldCheck, KeyRound, AlertTriangle, LogOut } from "lucide-react";
...
export default async function SignInPage({
  ...
  searchParams?:
    | Promise<{ redirect_url?: string; fallback_redirect_url?: string }>
    | { redirect_url?: string; fallback_redirect_url?: string };
}) {
  const resolvedParams = searchParams ? await Promise.resolve(searchParams) : {};
  const redirectUrl =
    resolvedParams.redirect_url || resolvedParams.fallback_redirect_url || "";

  // 1. If Clerk is configured and user already has an active session:
  if (isClerkReady) {
    try {
      const authData = auth();
      if (authData.userId) {
        // Check if user is an authorized Zonal Staff or Head
        const staffSession = await getZonalStaffSession();
        if (staffSession) {
          // Immediately redirect authorized staff to their station console!
          redirect("/staff");
        }

        // If user came with a redirect_url that isn't staff or sign-in, redirect them there
        if (
          redirectUrl &&
        ...
        // User is signed in to Clerk, but their account is NOT assigned to any zone
        const user = await currentUser();
        const currentEmail =
          user?.emailAddresses?.[0]?.emailAddress || "Current Account";
        ...
        return (
          <div className="min-h-screen bg-background text-foreground flex flex-col justify-center items-center p-4 selection:bg-primary selection:text-primary-foreground">
            ...
            {/* Account Status Card */}
            <div className="w-full max-w-md p-6 sm:p-8 bg-card text-card-foreground border-2 border-border shadow-neo space-y-5">
              <div className="flex items-center space-x-3 text-warning">
                <div className="w-10 h-10 bg-amber-500/10 border-2 border-border flex items-center justify-center text-amber-500">
              ...
              <div className="p-4 bg-muted border-2 border-border space-y-2 text-xs">
                <p className="font-bold text-foreground">
                  Staff Station Authorization:
                </p>
                <p className="text-muted-foreground leading-relaxed">
                  This account is active, but is not currently assigned to an Oceanic Zone by the event administrator.
                </p>
                <p className="text-muted-foreground leading-relaxed">
                  If you are a Zonal Head, please click below to switch accounts and sign in with the email assigned to your zone station.
                </p>
              </div>
              ...
              <div className="space-y-3 pt-2">
                ...
                >
                  <span>Continue to Attendee Portal</span>
      ...
      // If NEXT_REDIRECT was thrown by redirect(), rethrow it so Next.js performs the redirect!
      if (err?.digest?.startsWith("NEXT_REDIRECT")) {
        throw err;
      }
      console.warn("Clerk session check non-fatal error:", err);
    }
  }
        ...
        {isClerkReady ? (
          <div className="flex justify-center">
            <SignIn
              ...
              routing="path"
              signUpUrl="/sign-up"
              fallbackRedirectUrl={redirectUrl || "/staff"}
              forceRedirectUrl={redirectUrl ? redirectUrl : undefined}
              appearance={{
                elements: {
                  rootBox: "w-full",
