  ...
  try {
    ...
    if (isUsingLiveSupabase() && supabaseAdmin) {
      try {
        ...
        if (error) {
          console.warn("Supabase stall_photos insert error, falling back to mockDb:", error.message);
        } else if (data) {
          return { success: true, submission: data, message: "Photo submitted for volunteer verification!" };
        }
      } catch (err: any) {
        console.warn("Supabase stall_photos error, using mockDb fallback:", err.message);
      }
    }
  ...
  try {
    ...
    if (isUsingLiveSupabase() && supabaseAdmin) {
      try {
        ...
        console.warn("Supabase approveStallPhoto error, falling back to mockDb:", err.message);
      }
    }
  ...
  try {
    ...
    if (isUsingLiveSupabase() && supabaseAdmin) {
      try {
        ...
        console.warn("Supabase rejectStallPhoto error, falling back to mockDb:", err.message);
      }
    }
